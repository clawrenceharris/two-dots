...
public class SquareManager
{
    ...
    /* Returns the appropriate square based on conditions
     that check if it is a blank square or normal square */
    ...
    public void SelectDotsForSquare()
    {
...
public class Square
{
    ...
    private void FindDotsInsideSquare()
    {
        ...
        for (int col = 0; col < board.Width; col++)
        {
            ...
            {
                Dot dot = board.GetDotAt(col, row);
    ...
    // Selects the dots that would be hit by the square if it was released by the user.
    // Also adds them to a list so we don't have to loop through all dots again to hit them
    public virtual void SelectAndAddDotsForSquare()
    {
        for (int col = 0; col < board.Width; col++)
        {
            for (int row = 0; row < board.Height; row++)
